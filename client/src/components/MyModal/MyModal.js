...
const MyModal = (props) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.patch(`http://localhost:5000/api/v1/auth/update/${user._id}`, payload)
      .then(res => {
            ...
            <div className='cipher__modal-formContent'>
              <form className='cipher__modal-form' onSubmit={handleSubmit}>
                <div className='cipher__modal-formItem'>
                  ...
                  <input className="cipher__modal-input" type="text" name="username" placeholder="Enter the username" onChange={handleChange} />
                </div>
                <div className='cipher__modal-formItem'>
                  <label className='cipher__modal-label'>Email</label>
                  <input className="cipher__modal-input" type="email" name="email" placeholder="Enter the email" onChange={handleChange} />
                </div>
                <div className='cipher__modal-formItem'>
                  <label className='cipher__modal-label'>Password</label>
                  <input className="cipher__modal-input" type="password" name="password" placeholder="Enter the password" onChange={handleChange} />
                </div>
                <div className='modal__cipher-buttons'>
                  <button className="modal__cipher-save" type="submit">
                    Save
                  </button>
                  <button className="modal__cipher-cancel" onClick={handleClose}>
                    Cancel
                  </button>
                </div>
              </form>
